...
import { submitLeadToAirtable, updateLeadInAirtable } from "../utils/airtableLeads";
...
export default function ConsultationModal({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const hasLeadData = currentUserData && (currentUserData.name || currentUserData.email || currentUserData.phone);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (hasExistingLead && existingRecordId) {
        ...
        if (result.success) {
          console.log("✅ Consultation request updated in Airtable successfully");
        ...
        if (result.success && result.record?.id) {
          console.log("✅ Consultation request submitted to Airtable successfully");
  ...
  const practiceName =
    practice === "lakeshore" ? "Lakeshore Skin + Body" : "Unique Aesthetics & Wellness";
        ...
        <div className="consultation-modal-content">
          {showConfirmation ? (
            <div className="consultation-confirmation">
              <div className="consultation-confirmation-icon" style={{
                background: practice === "lakeshore" ? "rgba(107, 138, 154, 0.1)" : "rgba(255, 162, 199, 0.15)",
              ...
              <p className="consultation-confirmation-message">
                Your request has been sent to {practiceName}. We'll be in touch soon to schedule your consultation.
              </p>
              <button
                type="button"
            ...
            <>
              ...
              >
                ...
                <div className="consultation-form-field">
                  ...
                  />
                  {emailError && (
                    <span className="consultation-input-error-message">{emailError}</span>
                  )}
                </div>
                {!hasExistingLead && (
                  <div className="consultation-form-field">
                    ...
                    />
                    {phoneError && (
                      <span className="consultation-input-error-message">{phoneError}</span>
                    )}
                  </div>
                )}
                <button
