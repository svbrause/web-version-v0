...
const WELLNESS_QUIZ_STORAGE_KEY = "wellnessQuizResult";

type QuizPhase = "intro" | "questions" | "results";

export interface WellnessQuizModalProps {
  onClose: () => void;
  /** Optional: persist result to localStorage for test */
...
export default function WellnessQuizModal({
  ...
  const performSave = (answersToSave: Record<string, number | number[]>): boolean => {
    if (!persistToStorage) return true;
    try {
      const payload = buildWellnessQuizPayload(answersToSave);
      localStorage.setItem(WELLNESS_QUIZ_STORAGE_KEY, JSON.stringify(payload));
      return true;
    } catch {
      return false;
    }
  ...
  const handleSaveResults = (e: FormEvent) => {
    e.preventDefault();
    setSaveFailed(false);
    setLoading(true);
    const ok = performSave(answers);
    setLoading(false);
    if (!ok) setSaveFailed(true);
  };
  ...
  const modalContent = (
    <div
      className="consultation-modal-overlay wellness-quiz-modal-overlay"
      onClick={onClose}
      role="dialog"
      aria-modal="true"
    ...
    >
      <div
        className="wellness-quiz-modal-content consultation-modal"
