...
export default function ConcernsScreen() {
  ...
  const sortedConcerns = useMemo(() => {
    ...
    return [...HIGH_LEVEL_CONCERNS].sort((a, b) => {
      const aCount = getMatchingCasesForConcern(a.id, caseData, minimalState as any).length;
      const bCount = getMatchingCasesForConcern(b.id, caseData, minimalState as any).length;
      return bCount - aCount; // Descending order (most cases first)
    });
  }, [caseData, state.ageRange, state.skinType, state.skinTone, state.ethnicBackground]);
  ...
  return (
    <div className="question-view">
      <h1 className="main-question">What would you like to improve?</h1>
      <p className="subtitle">Select up to 3 concerns that matter most to you.</p>
        ...
        <div className="selection-warning">
          ...
          <div className="selection-warning-content">
            <p className="selection-warning-text">
              You can only select up to 3 concerns in this virtual consultation. For personalized recommendations tailored to your specific needs, 
              <button 
          ...
          return (
            ...
            >
              <div className="concern-card-content">
                ...
                <p className="concern-card-description">{concern.description}</p>
              </div>
            </button>
          );
        })}
