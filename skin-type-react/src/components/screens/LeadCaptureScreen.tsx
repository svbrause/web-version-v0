...
function isValidZipCode(zipCode: string): boolean {
  ...
  // Must be exactly 5 digits (or 9 digits for 5+4 format, but we'll accept 5 minimum)
...
function formatZipCode(value: string): string {
  ...
  // Limit to 9 digits (5+4 format)
  const zipDigits = digits.slice(0, 9);
...
export default function LeadCaptureScreen() {
  ...
  const handleZipCodeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Only clear error if they fix it while typing, don't show new errors
    if (zipCodeError && formatted && isValidZipCode(formatted)) {
      setZipCodeError("");
    }
  };
      ...
      <div className="lead-capture-container">
        ...
        >
          ...
          <div className="lead-form-field">
            <input
              type="text"
              id="earlyCaptureZipCode"
              name="zipCode"
              placeholder="Zip Code"
              required
              className={`lead-input ${zipCodeError ? "lead-input-error" : ""}`}
              value={zipCode}
            ...
            />
            {zipCodeError && (
              <span className="lead-input-error-message">{zipCodeError}</span>
            )}
          </div>
