...
export function getAvailableAreasForConcerns(
  ...
  // but we'll log a warning and return all areas as fallback
  if (caseData.length === 0) {
    console.warn('⚠️ getAvailableAreasForConcerns: caseData is empty, cannot filter areas. Returning all areas as fallback.', {
      selectedConcerns,
  ...
  selectedConcerns.forEach(concernId => {
    ...
    console.log(`📊 getAvailableAreasForConcerns: Concern ${concernId} has ${matchingCases.length} matching cases`);
  ...
  // If no areas found after processing all concerns, log warning but still return all areas (fallback)
  if (availableAreaIds.size === 0) {
    console.warn('⚠️ getAvailableAreasForConcerns: No areas found for selected concerns, returning all areas as fallback', {
      selectedConcerns,
      caseDataLength: caseData.length,
  ...
  console.log(`✅ getAvailableAreasForConcerns: Found ${availableAreaIds.size} available areas for ${selectedConcerns.length} concern(s)`);
