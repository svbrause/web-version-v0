...
export async function submitLeadToAirtable(
  ...
  try {
    // Use secure API route instead of direct Airtable call
    const API_URL = '/api/airtable-leads';
    ...
    const concernsExploredIds = behavioralData?.concernsExplored || getConcernsExplored();
    ...
    try {
      if (caseData.length > 0 && userState.selectedConcerns && userState.selectedConcerns.length > 0) {
        ...
        userState.selectedConcerns.forEach(concernId => {
          try {
            const matchingCases = getMatchingCasesForConcern(concernId, caseData, userState as AppState);
            matchingCases.forEach(c => allMatchingCaseIds.add(c.id));
          } catch (e) {
            console.warn(`Error calculating cases for concern ${concernId}:`, e);
          }
        });
    ...
    console.log('🔍 Concerns field type check:', Array.isArray(fields['Concerns']) ? 'Array ✓' : `Not array: ${typeof fields['Concerns']}`);
    console.log('🔍 Areas field type check:', Array.isArray(fields['Areas']) ? 'Array ✓' : `Not array: ${typeof fields['Areas']}`);
    ...
    if (!response.ok) {
      ...
      if (result.details?.error) {
        ...
        if (error.type === 'INVALID_VALUE_FOR_COLUMN') {
          console.error('   ⚠️ Field name mismatch! Check that field names in Airtable match exactly:');
          console.error('   Expected fields:', Object.keys(fields));
        } else if (error.type === 'TABLE_NOT_FOUND') {
          console.error('   ⚠️ Table not found! Check that table name is exactly: "Web Popup Leads"');
        } else if (error.type === 'AUTHENTICATION_REQUIRED') {
          console.error('   ⚠️ Authentication failed! Check your server-side API key configuration.');
        }
      }
      ...
      if (retryResponse.ok && retryResult.success) {
        console.log('✅ Lead submitted with minimal fields:', retryResult.record);
    ...
    if (result.success && result.record) {
      console.log('✅ Lead submitted successfully:', result.record);
      console.log('📊 Record created with fields:', result.record.fields || 'No fields in response');
      console.log('📊 Record ID:', result.record.id || 'No ID in response');
...
export async function updateLeadInAirtable(
  ...
  try {
    // Use secure API route instead of direct Airtable call
    const API_URL = '/api/airtable-update-lead';
    ...
    console.log('✅ Lead updated successfully:', result.record);
    return { success: true, record: result.record };
  } catch (error: any) {
