...
**Critical**: This app uses the **backend API** at `ponce-patient-backend.vercel.app` - API keys stay secure on the backend!

1. Go to **Settings** → **Environment Variables**
2. Add these variables (make sure to select **Production**, **Preview**, and **Development**):

```
VITE_PRACTICE_NAME=unique  # or 'lakeshore' - same as production
VITE_BACKEND_API_URL=https://ponce-patient-backend.vercel.app  # Backend API URL
VITE_USE_BACKEND_API=true  # Use backend API (default, can omit)
VITE_POSTHOG_KEY=your_posthog_api_key_here  # Optional
```

**⚠️ IMPORTANT - DO NOT SET THESE:**
- ❌ `VITE_AIRTABLE_API_KEY` - **Never set this!** It would expose your API key to the client
- ❌ `VITE_AIRTABLE_BASE_ID` - Not needed, backend handles this
- ❌ `AIRTABLE_API_KEY` - Not needed in this project, backend has it
- ❌ `AIRTABLE_BASE_ID` - Not needed in this project, backend has it

**Why?** The React app calls the backend API, which has the API keys securely stored. This way:
- ✅ API keys never exposed to client-side code
- ✅ Same backend as your other frontend projects
- ✅ Centralized key management
- ✅ Consistent architecture

**Note**: The backend at `ponce-patient-backend.vercel.app` already has:
- `AIRTABLE_API_KEY` (set in backend's Vercel project)
- `AIRTABLE_BASE_ID` (set in backend's Vercel project)
- `FRONTEND_URL` (includes your staging domain for CORS)
...
**This app uses the backend API** - you don't need Airtable API keys in the staging Vercel project!

**Required Variables:**
- `VITE_PRACTICE_NAME` - `unique` or `lakeshore`
- `VITE_BACKEND_API_URL` - `https://ponce-patient-backend.vercel.app` (default, can omit)
- `VITE_USE_BACKEND_API` - `true` (default, can omit)

**Optional:**
- `VITE_POSTHOG_KEY` - For analytics (optional)

**Backend Configuration:**
The backend at `ponce-patient-backend.vercel.app` needs:
- `AIRTABLE_API_KEY` - Already set in backend's Vercel project
- `AIRTABLE_BASE_ID` - Already set in backend's Vercel project  
- `FRONTEND_URL` - Must include your staging domain for CORS

**To add staging domain to backend's FRONTEND_URL:**
1. Go to backend's Vercel project (`ponce-patient-backend`)
2. Settings → Environment Variables
3. Update `FRONTEND_URL` to include staging URL:
   ```
